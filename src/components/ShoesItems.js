...
const ShoesItems = ({item ,  navigation }) => {
  ...
  return (
    ...
    <View style={{display:"flex" , flexDirection:"row" , justifyContent:"space-between" , marginBottom: pixelSizeHorizontal(20)}}>
        <Image style={{width:77 , height:64}} source={{uri : item.items.image.original}}></Image>
        <View style={{gap:6}}>
            <Text style={{fontSize:fontPixel(16) , fontWeight:"bold"}}>{`${item.items.brand} ${item.items.name}`}</Text>
            <Text style={{fontSize:fontPixel(14)}}>{`250km/${item.items.estimateKm}km`} </Text>
        </View>
